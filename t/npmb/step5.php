...
echo
  	'<div id="peng">'.
  		...
  		'</div>'.
  		'<div class="peng-g">'.
			'<h2>Pemberkasan & Daftar Ulang</h2>'.
			'<p>Kirimkan berkas di bawah ini dalam map kertas berwarna <strong>'.$map.'</strong> ke kantor ITM Nganjuk pada jam kerja paling lambat tanggal <strong>'.indotime(date('Y-m-d', strtotime($tgluj." + 2 weeks"))).'</strong>.</p><ul>';
			// '<p><strong>A.</strong> Kirimkan berkas di bawah ini dalam map kertas berwarna <strong>'.$map.'</strong> ke kantor ITM Nganjuk pada jam kerja paling lambat tanggal <strong>'.indotime(date('Y-m-d', strtotime($tgluj." + 2 weeks"))).'</strong>.</p><ul>';
			foreach($brks as $br){
				echo '<li>'.$br.'</li>';
			}
			echo '</ul>'.
