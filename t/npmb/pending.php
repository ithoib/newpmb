<?php 
$header_script .= '<script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.0.0/jquery.min.js"></script>';
$header_script .= '<link href="https://cdn.datatables.net/v/dt/jszip-2.5.0/dt-1.13.4/b-2.3.6/b-colvis-2.3.6/b-html5-2.3.6/b-print-2.3.6/r-2.4.1/datatables.min.css" rel="stylesheet"/>';
$header_script .= '<script src="https://cdn.datatables.net/v/dt/jszip-2.5.0/dt-1.13.4/b-2.3.6/b-colvis-2.3.6/b-html5-2.3.6/b-print-2.3.6/r-2.4.1/datatables.min.js"></script>';
$header_script .= '<script src="https://cdnjs.cloudflare.com/ajax/libs/pdfmake/0.2.7/pdfmake.min.js"></script>';
$header_script .= '<script src="https://cdnjs.cloudflare.com/ajax/libs/pdfmake/0.2.7/vfs_fonts.js"></script>';
include 'header.php';
if($s_role==1){
echo
'<div id="c">'.
	'<div class="ct">'.
		'<h2><i class="fas fa-users"></i> Data Mahasiswa Program Afiliasi</h2>'.
	'</div>';
	if($w=='tambah'){
		if($m=='ada'){
			echo '<div class="cw">Data sudah ada. Jangan menambahkan lebih dari 1 kali! <span class="cl"><i class="fas fa-times"></i></span></div>';
		} elseif($m=='ok'){
			echo '<div class="co">Akun berhasil diaktifkan! <span class="cl"><i class="fas fa-times"></i></span></div>';
		}
	} elseif($w=='ubah'){
		if($m=='ok'){
			echo '<div class="co">Data berhasil diubah! <span class="cl"><i class="fas fa-times"></i></span></div>';
		}
	} elseif($w=='hapus'){
		if($m=='ok'){
			echo '<div class="co">Data berhasil dihapus! <span class="cl"><i class="fas fa-times"></i></span></div>';
		}
	}
	$modal = '';
	if(count($pending)>0){
		echo
		'<table id="mhs" class="display dt-responsive nowrap" style="font-size:14px;width:100%;">'.
			'<thead>'.
				'<tr>'.
					'<th>No.</th>'.
					'<th>Tgl Daftar</th>'.
					'<th>Nama</th>'.
					'<th>Asal Sekolah</th>'.
					'<th>Afiliator</th>'.
					'<th>Status</th>'.
					'<th style="width:180px" class="notexport">Aksi</th>'.
				'</tr>'.
			'</thead>'.
			'<tbody>';
			$i = 0;
		foreach($pending as $mhs){
			$i++;

			echo '<tr>'.
					'<td><a href="'.ADMIN_URL.'/pendaftar/profil/'.$mhs['kode_reg'].'">'.$mhs['kode_reg'].'</a></td>'.
					'<td>'.$mhs['tgl_daftar'].'</td>'.
					'<td>'.$mhs['nama'].'</td>'.
					'<td>'.$mhs['asal_sekolah'].'</td>'.
					'<td>'.$mhs['refferer'].'</td>'.
					'<td>'.($mhs['status']==1 ? '<span class="cak"><i class="fas fa-check"></i> <em>Aktif</em></span>' : '<span class="ctk"><i class="fas fa-exclamation"></i> <em>Pending</em></span>').'</td>'.
					'<td>'.
						'<a href="#bukti'.$mhs['kode_reg'].'" class="cbb" rel="modal:open"><i class="fas fa-eye"></i></a>'.
						($mhs['status'] == 0 ? ' <a href="#aktif'.$mhs['kode_reg'].'" class="cbw" title="Aktifkan User" rel="modal:open"><i class="fas fa-check"></i></a>' : '').
					'</td>'.
				'</tr>';
				$modal .=
					'<div id="aktif'.$mhs['kode_reg'].'" class="modal">'.
						'<div class="modal-content">'.
							'<div class="modal-header">'.
								'<a href="#" class="close" rel="modal:close">&times;</a>'.
								'<h2>Hapus Pendaftar</h2>'.
							'</div>'.
							'<div class="modal-body">'.
								'<p>Apa Anda yakin akan mengaktifkan pendaftar atas nama '.$mhs['nama'].'?</p>'.
								'<form method="post" action="">'.
									'<input type="hidden" name="aktif[kode_reg]" value="'.$mhs['kode_reg'].'">'.
									'<input type="hidden" name="aktif[wa]" value="'.$mhs['wa'].'">'.
									'<div class="cbu">'.
										'<a href="#" rel="modal:close"><i class="fas fa-undo-alt"></i> Tidak</a>'.
										'<button type="submit" class="ya"><i class="fas fa-check"></i> Ya</button>'.
									'</div>'.
								'</form>'.
							'</div>'.
						'</div>'.
					'</div>';
				$modal .=
					'<div id="bukti'.$mhs['kode_reg'].'" class="modal">'.
						'<div class="modal-content">'.
							'<div class="modal-header">'.
								'<a href="#" class="close" rel="modal:close">&times;</a>'.
								'<h2>Bukti Pembayaran</h2>'.
							'</div>'.
							'<div class="modal-body">'.
								'<img src="'.SITEURL.'/'.$mhs['bukti'].'" alt="Bukti Pembayaran '.$mhs['kode_reg'].'" style="width:100%" loading="lazy">'.
							'</div>'.
						'</div>'.
					'</div>';
		}
		echo '</tbody>'.
			'</table>';
	} else {
		echo '<div class="cw">Tidak ada data <span class="cl"><i class="fas fa-times"></i></span></div>';
	}
echo '</div>'.
$modal;
$footer_script .= '<script src="'.THEME_URL.'/a/modal.js"></script>';
$footer_script .= '<script src="'.THEME_URL.'/a/close.js"></script>';
$footer_script .=
'<script>'.
'$(document).ready( function () {'.
'$(\'#mhs\').DataTable({'.
'responsive: true,'.
'columnDefs: ['.
'{ responsivePriority: 1, targets: 0 },'.
'{ responsivePriority: 2, targets: -1 }'.
'],'.
'language: { search: "", searchPlaceholder: "Cari data" },'.
'pageLength : 50,'.
'order: [],'.
'dom: "Bfrtip",'.
'buttons: ['.
'{'.
'extend: "excel",'.
'text: \'<i class="far fa-file-excel"></i> Excel\','.
'className: "bexc",'.
'title: "Data Pendaftar Program Afiliasi Per Tanggal '.date('d-m-Y H:i:s').'",'.
'exportOptions: {'.
'columns: [":not(.notexport)"],'.
'}'.
'},'.
'{'.
'extend: "pdf",'.
'text: \'<i class="far fa-file-pdf"></i> PDF\','.
'title: "Data Pendaftar Program Afiliasi Per Tanggal '.date('d-m-Y H:i:s').'",'.
'className: "bpdf",'.
'exportOptions: {'.
'columns: ":not(.notexport)"'.
'}'.
'},'.
'{'.
'extend: "print",'.
'text: \'<i class="fas fa-print"></i> Print\','.
'title: "Data Pendaftar Program Afiliasi Per Tanggal '.date('d-m-Y H:i:s').'",'.
'className: "bprn",'.
'exportOptions: {'.
'columns: [":not(.notexport)"]'.
'}'.
'},'.
'],'.
'});'.
'} );'.
'</script>';
} else {
	echo
	'<div id="c">'.
		'<div class="ct">'.
			'<h2><i class="fas fa-users"></i> Data Mahasiswa Program Afiliasi</h2>'.
		'</div>'.
		'<div class="cw">Anda tidak berhak mengakses halaman ini! <span class="cl"><i class="fas fa-times"></i></span></div>'.
	'</div>';
}
include 'footer.php';
?>